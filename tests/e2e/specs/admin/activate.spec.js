...
test.describe( 'Plugin activation', () => {
	test( 'Can deactivate the plugin', async ( { admin, page } ) => {
		...
		await page
			.locator( '#deactivate-ai-provider-for-azure-openai' )
			.click();
	...
	test( 'Can activate the plugin', async ( { admin, page } ) => {
		await visitAdminPage( admin, 'plugins.php' );
		await page.locator( '#activate-ai-provider-for-azure-openai' ).click();
	...
	test( 'Can see Settings links in the plugin action links', async ( {
		...
		await expect(
			page.locator(
				'tr[data-slug="ai-provider-for-azure-openai"] .row-actions a',
				{
					hasText: 'Settings',
				}
			)
